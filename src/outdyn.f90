subroutine outdyn(displ, dtime, istep, ndofn, nelem, ngaus, ngrqs, noutd, noutp, npoin, nprqd, nreqd, nreqs, ntype, strsg, tdisp,  &
    vivel)
!
! OUTPUT ROUTINE
!
    ...
    integer :: istep, nstr1, kstep, mgaus, ndofn, nelem, ngaus, npoin, ntype, kount, koutd, nposn, nposm, ipoin, ireqd, ikoun,     &
        igaus, istr1, ireqs, noutd, noutp, nreqd, nreqs, nodet, idofn, ngasi, ngasj, ngask, mgasj, mgask, jpoin, kpoin, igasi,     &
        mgasi, igasj, igask, kgaus, ielem, kelgs, jgaus, istre

    integer :: ngrqs(1)

    real :: dtime, ttime, xtime, xgash, xgish, xgesh, xgosh
    real :: displ(1), strsp(3), vivel(5,1),tdisp(1), strsg(4,1) ,nprqd(1)
    ...
    ! wrtes displacment history at requested nodal points on tape 10
...
510 koutd=(kstep/noutp ) *noutp
    ...
    do ipoin = 1, npoin,3
        ...
        ! writeS displacements on the tape 13 for deformation plot
